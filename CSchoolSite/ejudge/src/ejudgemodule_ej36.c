...
Ul_do_login(int cmd, UlObject *self, PyObject *args)
{
  ...
  if ((r = userlist_clnt_login(self->clnt, cmd, &ip_val, sid, ssl_flag, contest_id,
                               locale_id, 0, login, password,
                               &uid, &sid, &clkey, &name)) < 0) {
    if (r < -1) PyErr_SetString(PyExc_IOError, userlist_strerror(-r));
    return 0;
  }

  val = Py_BuildValue("{s:i,s:K,s:K,s:s}",
                      "user_id", uid, "sid", sid, "client_key", clkey, "name", name);
...
Ul_do_get_cookie(int cmd, UlObject *self, PyObject *args)
{
  ...
  if (!PyArg_ParseTuple(args, "siKK", &ip_str, &ssl_flag, &sid, &clkey))
    return 0;
  if (xml_parse_ipv6(NULL, 0, -1, 0, ip_str, &ip_val) < 0) {
    PyErr_SetString(PyExc_ValueError, "invalid IP");
    return 0;
  ...
  if ((r = userlist_clnt_get_cookie
       (self->clnt, cmd, &ip_val, ssl_flag, sid, clkey,
...
Ul_logout(UlObject *self, PyObject *args)
{
  ...
  if ((r = userlist_clnt_logout(self->clnt, ULS_DO_LOGOUT, &ip_val, ssl_flag, sid, clkey)) < 0) {
    if (r < -1) PyErr_SetString(PyExc_IOError, userlist_strerror(-r));
    return 0;
  }
  return Py_BuildValue("i", 0);
...
Ul_do_priv_login(int cmd, UlObject *self, PyObject *args)
{
  ...
  if ((r = userlist_clnt_priv_login(self->clnt, cmd, &ip_val, sid, ssl_flag, contest_id, locale_id, role, login, password, &user_id, &sid, &clkey, &priv_level_out, &name)) < 0) {
    if (r < -1) PyErr_SetString(PyExc_IOError, userlist_strerror(-r));
    return 0;
  }
  val = Py_BuildValue("{s:i,s:K,s:K,s:i,s:s}","user_id", user_id, "sid", sid, "client_key", clkey, "priv_level", priv_level_out, "name", name);
  free(name);
  return val;
}
...
Ul_do_set_cookie(int cmd, UlObject *self, PyObject *args)
{
  ...
  if (!PyArg_ParseTuple(args, "KKi", &sid, &clkey, &locale_id))
    return 0;
  if ((r = userlist_clnt_set_cookie(self->clnt, cmd, sid, clkey, locale_id)) < 0) {
    if (r < -1) PyErr_SetString(PyExc_IOError, userlist_strerror(-r));
    return 0;
  }
  return Py_BuildValue("i", 0);
...
Ul_deleteCookie(UlObject *self, PyObject *args)
{
  ...
  if (!PyArg_ParseTuple(args, "iiKK", &user_id, &contest_id, &sid, &clkey))
    return 0;
...
Ul_loginBySID(UlObject *self, PyObject *args)
{
  ...
  if ((r = userlist_clnt_lookup_cookie
       (self->clnt, &ip_val, ssl_flag, sid, clkey,
...
Ul_contestLoginBySID(UlObject *self, PyObject *args)
{
  ...
  if (!PyArg_ParseTuple(args, "siiKK", &ip_str, &ssl_flag, &contest_id, &sid, &clkey))
    return 0;
  if (xml_parse_ipv6(NULL, 0, -1, 0, ip_str, &ip_val) < 0) {
    PyErr_SetString(PyExc_ValueError, "invalid IP");
    return 0;
  ...
  if ((r = userlist_clnt_team_cookie
       (self->clnt, &ip_val, ssl_flag, contest_id, sid, clkey,
...
Ul_privLoginBySID(UlObject *self, PyObject *args)
{
  ...
  if (!PyArg_ParseTuple(args, "siiKKi", &ip_str, &ssl_flag, &contest_id, &sid, &clkey, &priv_level))
    return 0;
  if (xml_parse_ipv6(NULL, 0, -1, 0, ip_str, &ip_val) < 0) {
    PyErr_SetString(PyExc_ValueError, "invalid IP");
    return 0;
  ...
  if ((r = userlist_clnt_priv_cookie
       (self->clnt, &ip_val, ssl_flag, contest_id, sid, clkey, priv_level,
...
static struct PyModuleDef ejudgemodule = {
